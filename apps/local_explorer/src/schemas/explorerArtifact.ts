...
const interventionExperimentSchema = z.object({
  ...
  feature: z.object({
    ...
    conceptLabel: z.string().min(1).nullable().optional(),
    conceptSource: z.enum(["neuronpedia", "index"]).optional(),
    conceptUrl: z.string().url().nullable().optional(),
  ...
  vector: z.object({
    ...
    positivePrompts: z.array(z.string().min(1)).min(1).optional(),
    negativePrompts: z.array(z.string().min(1)).min(1).optional(),
    positiveCount: z.number().int().positive().optional(),
    negativeCount: z.number().int().positive().optional(),
    activationReduce: z.string().min(1),
    rawNorm: z.number().finite().positive(),
    normalized: z.boolean(),
    dimension: z.number().int().positive(),
    sourceKey: z.string().min(1),
    injectionKey: z.string().min(1).optional(),
    injectionPhase: z.enum(["generation", "prompt", "prompt_and_generation"]).optional(),
