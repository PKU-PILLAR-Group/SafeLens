              ...
              {run.layers.length <= 16 ? (
                <div className="layer-button-selector" role="radiogroup" aria-label="Analysis layer">
                  ...
                  <button
                    ...
                    onClick={() => selection.selectLayer(layer)}
                    onKeyDown={(event) => handleLayerKeyDown(event, run.layers, layer, selection.selectLayer)}
                  >
                    L{layer}
                  </button>
                  ))}
...
function WorkspaceTabs({
  ...
  React.useEffect(() => {
    ...
    tabList.scrollLeft = Math.max(0, (tabs[pageStart]?.offsetLeft ?? first.offsetLeft) - first.offsetLeft);
  ...
  return (
    <div className={`workspace-tabs-shell ${scrollState.previous ? "has-previous" : ""} ${scrollState.next ? "has-next" : ""}`}>
      <button
        key="previous-view-tabs"
        type="button"
...
function CompactLayerSelector({
  ...
  return <div className="compact-layer-selector">
    <button
